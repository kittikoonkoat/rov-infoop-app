...
def init_connection():
    ...
    try:
        # ใช้ข้อมูลจากไฟล์ rov-ya-mo-02665ab0b48a.json
        creds_dict = st.secrets["gcp_service_account"]
        creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
        client = gspread.authorize(creds)
        return client.open("RoV_Seeding_DB")
    except Exception as e:
        # แสดง Error หากข้อมูลใน Secrets ไม่ถูกต้อง
        st.error(f"เชื่อมต่อ Google Sheets ไม่ได้: {e}")
        return None

def sync_data():
    """ซิงค์ข้อมูลล่าสุดจาก Google Sheets"""
    sh = init_connection()
    if sh:
        try:
...
# ==========================================
# 2. AI AGENT CONNECTOR (FIXED FOR 10 OPTIONS)
# ==========================================

def call_ai_agent(topic, guide):
    """เรียกใช้ AI และแยกผลลัพธ์ออกเป็น 10 รูปแบบ"""
    ...
    try:
        ...
        # แกะข้อมูลจากโครงสร้าง JSON ของ INSEA AI
        raw_text = ""
        ...
        # แก้ไขปัญหา "AI คิดไม่ออก" โดยใช้ Regex แยกข้อความ
        # แยกด้วยการขึ้นบรรทัดใหม่ หรือตัวเลขข้อที่ AI อาจเผลอใส่มา
        options = [l.strip() for l in re.split(r'\n|\d+\.', str(raw_text)) if len(l.strip()) > 5]
        
        return options[:10] if options else ["AI ขัดข้อง ลองตรวจสอบหัวข้อใน Sheets นะคะ"]
        ...
        for t in my_jobs:
            with st.expander(f"📌 {t['Topic']} | สถานะ: {t['Status']}", expanded=True):
                if t['Status'] == "Pending":
                    ...
                    # แสดงรายการปุ่มเลือกแบบที่ AI ร่างมา
                    ...
                    st.success("✅ หัวหน้าอนุมัติแล้ว! คัดลอกไปโพสต์ได้เลย")
                    st.code(t['Draft'])
